...
function insertMsg (servId, channelName, from, msg) {
    ...
    var messageTable = view.getC('message-table');
    messageTable.appendChild(tr);
    fixLeftBufferWidth(nick, messageTable);
...
function fixLeftBufferWidth (box, messageTable) {setTimeout(function () {

    if (!messageTable.retrieve('leftWidth')) {
        messageTable.store('leftWidth', box.clientWidth);
        return;
    }

    if (messageTable.retrieve('leftWidth') >= box.clientWidth) {
        var leftWidth = messageTable.retrieve('leftWidth');
        leftWidth     = leftWidth - 10; // XXX !!!!
console.log(leftWidth);
        box.setStyle('width', leftWidth + 'px');
        return;
    }

    messageTable.store('leftWidth', box.clientWidth);
    var things = Array.from(messageTable.getElements('.asterisk, .nick'));
    things.each(function (thing) {
        thing.setStyle('width', box.clientWidth + 'px');
    });

    return true;    
}, 500); }

function channelNotify (servId, channelName, type, msg) {return;
